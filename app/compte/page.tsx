...
export default async function ComptePage() {
  ...
  return (
    <main className="auth-page">
      <div className="auth-visual"><Brand /><div><p className="eyebrow">Votre espace voyageur</p><h1>Tout est prêt pour votre séjour.</h1><p>Gardez vos informations à jour pour recevoir nos réponses rapidement sur WhatsApp.</p></div></div>
      <section className="auth-panel"><div className="account-bar"><SessionControls isAuthenticated /></div><Link className="auth-back" href="/">← Retour à l&apos;accueil</Link><div className="auth-heading"><p className="eyebrow">Espace client</p><h2>{complete ? 'Votre profil' : 'Complétez votre profil'}</h2><p>{complete ? 'Vos informations sont prêtes pour vos demandes de réservation.' : 'Ces informations sont obligatoires avant toute demande de réservation.'}</p></div><div className="client-panel-links"><Link href="/appartements">Explorer les appartements <span>↗</span></Link><Link href="/reservations">Mes demandes et réservations <span>↗</span></Link></div><ProfileCompletionForm initialValues={values} /><p className="auth-switch"><Link href="/appartements">Voir les appartements ↗</Link></p></section>
    </main>
  );
}

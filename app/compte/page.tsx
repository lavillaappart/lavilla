...
export default async function ComptePage() {
  ...
  return (
    <main className="auth-page">
      ...
      <section className="auth-panel"><Link className="auth-back" href="/">← Retour à l&apos;accueil</Link><div className="auth-heading"><p className="eyebrow">Mon compte</p><h2>{complete ? 'Votre profil' : 'Complétez votre profil'}</h2><p>{complete ? 'Vos informations sont prêtes pour vos demandes de réservation.' : 'Ces informations sont obligatoires avant toute demande de réservation.'}</p></div><ProfileCompletionForm initialValues={values} /><p className="auth-switch"><Link href="/appartements">Voir les appartements ↗</Link></p></section>
    </main>
  );
}

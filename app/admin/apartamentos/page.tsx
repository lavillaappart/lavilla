...
export default async function AdminApartamentosPage() {
  ...
  const { data: apartments } = await supabase.from('apartments').select('id, slug, city, status, capacity, base_price, apartment_translations(name)').order('created_at', { ascending: false });

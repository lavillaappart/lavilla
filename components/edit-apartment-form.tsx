...
export default function EditApartmentForm({ apartment }: { apartment: ApartmentData }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    const updateResult = await supabase.from('apartments').update({ slug, city: form.city, address: form.address, capacity: Number(form.capacity), bedrooms: Number(form.bedrooms), beds: Number(form.beds), bathrooms: Number(form.bathrooms), surface_m2: Number(form.surface || 0), base_price: Number(form.price), currency: 'MAD', min_stay_nights: Number(form.minStay), max_stay_nights: form.maxStay ? Number(form.maxStay) : null, check_in_time: form.checkIn, check_out_time: form.checkOut }).eq('id', apartment.id);

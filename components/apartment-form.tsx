...
export default function ApartmentForm() {
  ...
  const addImages = (selectedFiles: FileList | null) => {
    ...
    setImages((current) => {
      const existing = new Set(current.map((image) => `${image.name}-${image.size}-${image.lastModified}`));
      return [...current, ...selected.filter((image) => !existing.has(`${image.name}-${image.size}-${image.lastModified}`))];
    });
  };
  ...
  return (
    <form className="apartment-form" onSubmit={handleSubmit}>
      ...
      <section className="form-section"><div className="form-section-heading"><span>03</span><div><h2>Horaires et photos</h2><p>Ajoutez plusieurs photos. La couverture peut être choisie parmi toutes les images.</p></div></div><div className="form-grid two"><label>Heure d’arrivée<input type="time" value={form.checkIn} onChange={(event) => update('checkIn', event.target.value)} required /></label><label>Heure de départ<input type="time" value={form.checkOut} onChange={(event) => update('checkOut', event.target.value)} required /></label><label className="upload-zone full"><span>Ajouter des photos à la galerie</span><input type="file" accept="image/jpeg,image/png,image/webp" multiple onChange={(event) => addImages(event.target.files)} /><small>{images.length ? `${images.length} photo(s) sélectionnée(s) · Vous pouvez en ajouter d’autres` : 'Cliquez ici et sélectionnez plusieurs fichiers avec Ctrl ou Shift'}</small></label>{images.length ? <div className="image-selection full"><p className="image-selection-label">Photo de couverture</p><div className="image-preview-grid">{images.map((image, index) => <label className={`image-preview ${index === coverIndex ? 'selected' : ''}`} key={`${image.name}-${index}`}><img src={URL.createObjectURL(image)} alt={image.name} /><span><input type="radio" name="cover-image" checked={index === coverIndex} onChange={() => setCoverIndex(index)} /> {index === coverIndex ? 'Couverture' : 'Galerie'}</span></label>)}</div></div> : null}</div></section>

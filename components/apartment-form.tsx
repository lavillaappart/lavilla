...
export default function ApartmentForm() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    const { data: apartment, error } = await supabase.from('apartments').insert({ slug, status: 'active', city: form.city, address: form.address, capacity: Number(form.capacity), bedrooms: Number(form.bedrooms), beds: Number(form.beds), bathrooms: Number(form.bathrooms), surface_m2: Number(form.surface || 0), base_price: Number(form.price), min_stay_nights: Number(form.minStay), max_stay_nights: form.maxStay ? Number(form.maxStay) : null, currency: 'EUR', check_in_time: form.checkIn, check_out_time: form.checkOut, created_by: userData.user?.id }).select('id').single();
